...
def process(peakID,parse,scores):
    chipfile=CHIP_DIR+"/"+CHIP_NAME+"."+TIME+"."+peakID+".fastb"
    peaks=getPeaks(parse)
    annotateMotifs(peaks,peakID)
    fastb=Fastb(chipfile)
    getCHIPscores(peaks,fastb)
    numPeaks=len(peaks)
    for peak in peaks:
...
filename="cebp-sums-motif.txt" if MOTIF_ONLY else "cebp-sums.txt"
