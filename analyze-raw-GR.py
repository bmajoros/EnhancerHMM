...
MOTIF_ONLY=True
MARGIN=100
GR_DIR="/data/reddylab/projects/GGR/data/hmm/preprocessing/EP300_not_standardized/split_fastb"
...
def annotateMotifs(peaks,peakID):
    motifs=getMotifs("GR/AR/MR",peakID)
    for peak in peaks:
        peak.motif=False
        interval=peak
...
def process(peakID,parse,scores):
    grfile=GR_DIR+"/GR."+TIME+"."+peakID+".fastb"
    peaks=getPeaks(parse)
    annotateMotifs(peaks,peakID)
    fastb=Fastb(grfile)
    getGRscores(peaks,fastb)
    numPeaks=len(peaks)
    for peak in peaks:
...
def getGRscores(peaks,fastb):
    track=fastb.getTrackByName("GR")
    for peak in peaks:
...
filename="gr-sums-motif.txt" if MOTIF_ONLY else "gr-sums.txt"
